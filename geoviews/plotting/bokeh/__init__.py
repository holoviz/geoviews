...
class TilePlot(GeoPlot):
    ...
    def get_extents(self, element, ranges, range_type="combined", **kwargs):
        ...
        if (
            not self.overlaid
            and all(e is None or not np.isfinite(e) for e in extents)
    ...
    def get_data(self, element, ranges, style):
        if not isinstance(element.data, (str, dict)):
            SkipRendering(
                f"WMTS element data must be a URL string, bokeh cannot render {element.data!r}"
            )
        ...
        if isinstance(element.data, dict):
            ...
            if all(kw in element.data.upper() for kw in ("{X}", "{Y}", "{Z}")):
                ...
                kw in element.data.upper() for kw in ("{XMIN}", "{XMAX}", "{YMIN}", "{YMAX}")
            ):
                tile_source = BBoxTileSource
            else:
                raise ValueError(
                    "Tile source URL format not recognized. "
