...
def get_cb_plot(cb, plot=None):
    ...
    if isinstance(plot, GeoOverlayPlot):
        ...
        plots = [
            p
            for p in plots
            if any(s in cb.streams and getattr(s, "_triggering", False) for s in p.streams)
        ]
        if plots:
            plot = plots[0]
    return plot
...
def skip(cb, msg, attributes):
    ...
    if not all(a in msg for a in attributes):
        return True
    plot = get_cb_plot(cb)
    return not getattr(plot, "geographic", False) or not hasattr(plot.current_frame, "crs")
...
class GeoSelectionXYCallback(SelectionXYCallback):
    def _process_msg(self, msg):
        msg = super()._process_msg(msg)
        if skip(self, msg, ("x_selection", "y_selection")) or not all(
            isinstance(sel, tuple) for sel in msg.values()
        ):
            return msg
...
class GeoBoundsXCallback(BoundsXCallback):
    def _process_msg(self, msg):
        msg = super()._process_msg(msg)
        if skip(self, msg, ("boundsx",)):
            return msg
        x0, x1 = msg["boundsx"]
        plot = get_cb_plot(self)
        x0, _, x1, _ = project_extents((x0, 0, x1, 0), plot.projection, plot.current_frame.crs)
...
class PolyVertexEditCallback(GeoPolyEditCallback):
    ...
    def _create_vertex_split_link(self, action, poly_renderer, vertex_renderer, vertex_tool):
        cb = CustomJS(
            ...
            args={"poly": poly_renderer, "vertex": vertex_renderer, "tool": vertex_tool},
        )
        action.callback = cb
