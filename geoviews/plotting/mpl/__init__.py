...
class GeoOverlayPlot(ProjectionPlot, HvOverlayPlot):
    ...
    def _finalize_axis(self, *args, **kwargs):
        gridlabels = self.geographic and isinstance(
            self.projection, (ccrs.PlateCarree, ccrs.Mercator)
        )
        if gridlabels:
...
class GeoShapePlot(GeometryPlot, PolygonPlot):
    ...
    def get_data(self, element, ranges, style):
        if self.geographic:
            ...
            SkipRendering(
                "Shape can only be plotted on geographic plot, "
...
class FeaturePlot(GeoPlot):
    ...
    scale = param.Selector(
        default="110m", objects=["10m", "50m", "110m"], doc="The scale of the Feature in meters."
    )

    style_opts = ["alpha", "facecolor", "edgecolor", "linestyle", "linewidth", "visible"]
...
class GeoAnnotationPlot(AnnotationPlot):
    ...
    def update_handles(self, key, axis, annotation, ranges, style):
        ...
        self.handles["annotations"] = self.draw_annotation(
            axis, annotation.data, annotation.crs, style
        )
