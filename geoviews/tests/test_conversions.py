...
class TestConversions:
    ...
    def test_is_geographic_2d(self):
        assert is_geographic(gv.Dataset(self.cube, kdims=['longitude', 'latitude']), ['longitude', 'latitude'])

    def test_geographic_conversion(self):
        assert_element_equal(gv.Dataset(self.cube, kdims=['longitude', 'latitude']).to.image(), gv.Image(self.cube, kdims=['longitude', 'latitude']))
